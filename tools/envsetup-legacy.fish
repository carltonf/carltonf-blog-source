# As Thu Aug 18 15:33:04: deprecated as we are migrating to `habit` tool.
# This file can be removed after migration done.
#
# Status: in migration, most functionality have been ported to `habit` or new
# `envsetup.fish`.
...
function __fish_jekyll_list_recent_drafts -d 'List the TWO most recent files from _drafts and _posts/'
  ...
  #
  # TODO only list most recent posts? (with a arg '-m 7')
